...
if (valid_array($CUR_Layout) == true and ($sl_max_rows > 0 || $sl_max_cols > 0)) {
	...
	foreach ($sl_bus_deck['deck_config'] as $deck_k => $deck_v) {
		...
		$seat_layout .= '<div class="upnddown"><table class="table table-condensed"><tbody class="ssss">';
		if ($deck_k == 'Lower') {
		...
		for ($col = $deck_v['max_col']; $col >= $deck_v['min_col']; $col--) {
			...
			for($row = $deck_v['min_row']; $row <= $deck_v['max_row']; $row++) {
				...
				if($deck_k == 'Upper'){
					...
					$first_fare = $sl_matrix['Upper'][$row][$col]['Fare'];
					$Markup = $fare_details['default_value'] - $first_fare;
					//update Fare
					...
					//update Fare
					$first_fare = $sl_matrix['Lower'][$row][$col]['Fare'];
					...
					$sl_matrix['Lower'][$row][$col]['Fare'] = $seat_no_summary[$seat_number]['Fare'] = $fare_details['default_value']+$gst_value;
					//Available
					$seat_type = '';
