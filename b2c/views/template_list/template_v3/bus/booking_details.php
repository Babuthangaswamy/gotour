...
///debug($data);exit;
	...
	$core_booking_details = $booking_details[0]['core_booking_details'];
	debug($core_booking_details);exit;
	$booking_id = $core_booking_details['app_reference'];
	exit;
	$booking_made_on = app_friendly_date($core_booking_details['created_datetime']);
	
	$choosen_parameters = $core_booking_details['itinerary_details'][0];
	$booking_status = $core_booking_details['status'];
		...
		<div class="row">
			...
			<div class="col-sm-<?php echo $col_md;?>">
				...
				<div class="row mt20 mb10 font12 bold">
						...
						<div class="col-md-3" style=" text-align: right; ">
							<div class="dropdown menu_action_holder" id="dLabel2" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
								<?php echo menu($core_booking_details); ?>
							</div> 
				...
				<div class="row bs-example bg_top_itry font12 p15-0">
					...
					<?php echo get_bus_pax_details($core_booking_details);  ?>
				</div>
				<!-- PASSENGER -->
			<div class="clear-fix"></div>
				<!-- payment summary -->
					...
					<div class="col-sm-6 pad0 detail_right_border">
						...
						<div class="passenger_border f18 color369">
							<div class="row">
								...
								<div class="col-sm-6 tr"><i class="fa fa-inr"></i> <?=$core_booking_details['grand_total']?></div>
							</div>
						</div>
					</div>
...
function get_bus_pax_details($core_booking_details)
{
	...
	if(count($core_booking_details['customer_details']) > 0){
		
		$booking_status = $core_booking_details['status'];
		$pnr = $core_booking_details['pnr'];
		
		foreach($core_booking_details['customer_details'] as $k => $v){
...
/** Menu **/
function menu($core_booking_details){
	$v = array();
	$v['app_reference'] = $core_booking_details['app_reference'];
	$v['booking_source'] = $core_booking_details['booking_source'];
	$v['status'] = $core_booking_details['status'];
	$customer_email = $core_booking_details['email'];
