					...
					<div class="trvlwrap">
						...
						if(valid_array($table_data['booking_details']) == true) {
							...
							foreach($booking_details as $parent_k => $parent_v) { 
								extract($parent_v);
								$attributes = json_decode($parent_v['attributes'],true);
									...
									<div class="bookrow">
										...
										<div class="remful">
											<div class="xlbook col-sm-2 col-xs-3 htlimage_xl">
												<div class="imghtlflt">
													<img src="<?=$attributes['ProductImage']?>" alt="HotelImage" />
												</div>
											</div>
											<div class="xlbook col-sm col-xs-9 htlimage_xl ">
												<div class="htlfltr">
													<h3 class="shtlname"><?=$product_name?></h3>
													<div class="shtlname"><?=app_friendly_absolute_date($travel_date)?></div>
													<div class="starrting">
											...
											<?php 
												$booking_amount = roundoff_number($parent_v['product_total_price']+ $parent_v['convinence_amount']-$parent_v['discount']);

													
											?>
											
												<div class="sideprice">
													<?=$currency?><?=$booking_amount?>
												</div>
...
{
	$action_tab  = '';
	$ticket_details = '<a href="'.base_url().'index.php/voucher/sightseeing/'.$app_reference.'/'.$booking_source.'" class="viwedetsb">View</a>';
	if($status == 'BOOKING_CONFIRMED') {
		$cancel_booking = '<a href="'.base_url().'index.php/sightseeing/pre_cancellation/'.$app_reference.'/'.$booking_source.'" class="viwedetsb">Cancel</a>';
		$cancel_booking .= '<form action="" method="POST" name="shareemail-form">
														...
														<input type="hidden" value="activities" name="module" />
