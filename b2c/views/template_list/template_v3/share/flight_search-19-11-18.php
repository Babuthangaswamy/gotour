...
if ((isset($flight_search_params) == false) || (isset($flight_search_params) == true && valid_array($flight_search_params) == false)) {
	...
	if ($sid > 0) {
		...
		if ($flight_search_params['trip_type'] != 'multicity' && strtotime(@$flight_search_params['depature']) < time() ) {
			$flight_search_params['depature'] = date('m-d-Y');
			if (isset($flight_search_params['return']) == true) {
				$flight_search_params['return'] = date('m-d-Y', strtotime(add_days_to_date(1)));
			}
		}
	}
}
...
if(@$flight_search_params['trip_type'] != 'multicity') {
	...
	$var = $onw_rndw_segment_search_params['depature'];	
	$date = str_replace('/', '-', $var);
	$onw_rndw_segment_search_params['depature']= date('m-d-Y', strtotime($date));


/*	echo "babutest";
	debug($onw_rndw_segment_search_params['depature']);	
	exit;*/
...
echo $this->current_page->set_datepicker($flight_datepicker);
...
<form autocomplete="off" name="flight" id="flight_form" action="<?php echo base_url();?>index.php/general/pre_flight_search" method="get" class="activeForm oneway_frm" style="">
	<div class="tabspl">
		<div class="tabrow">
			<div class="waywy">
				<div class="smalway">
					<!--backup <label class="wament hand-cursor">
					...
					<label class="wament hand-cursor">
						<input class="hide" type="radio" name="trip_type" <?=(isset($flight_search_params['trip_type']) == false ? 'checked' : ($flight_search_params['trip_type']) == 'circle' ? 'checked="checked"' : '')?> id="rnd-trp" value="circle" /> Roundtrip<span> / Return</span>
					</label>

					<label class="wament hand-cursor">
						<input class="hide" type="radio" name="trip_type" <?=(@$flight_search_params['trip_type'] == 'oneway' ? 'checked="checked"' : '')?> id="onew-trp" value="oneway" /> One way
					</label>
			...
			<div id="onw_rndw_fieldset" class="col-md-12 nopad"><!-- Oneway/Roundway Fileds Starts-->
				<div class="col-md-6 nopad placerows">
				...
				<div class="col-md-6 nopad secndates">
					...
					<div class="col-xs-6 padfive date-wrapper">
						...
						<div class="plcetogo datemark sidebord datepicker_new2" iditem="flight_datepicker2">
							<input type="text" readonly class="normalinput auto-focus hand-cursor form-control b-r-0" id="flight_datepicker2" name="return" placeholder="Select Date" value="<?php echo @date('m-d-Y',$onw_rndw_segment_search_params['return']); ?>" <?=(@$onw_rndw_segment_search_params['trip_type'] != 'circle' ? 'disabled="disabled"' : '')?> />
						</div>
					</div>
				</div>
			...
			<div class="col-md-6 col-xs-12 nopad thrdtraveller">
				<div class="col-xs-6 padfive mobile_width">
					<div class="lablform">Traveler</div>
					<div class="totlall">
						<span class="remngwd"><span class="total_pax_count"></span> <span id="">Traveler</span></span>
...
<!-- 						<select id="airline_select" style="width:300px;">
						...
						<div class="mobile_adult_icon">Traveler<i class="fa fa-male"></i></div>
						
							<div class="inallsn">
								<div class="oneroom fltravlr">
								<div class="lablform2">Traveler</div>
...
/*	var flight_datepicker1 = $('#flight_datepicker1').datepicker({ dateFormat: 'mm-dd-yy' }).val();

var flight_datepicker2 = $('#flight_datepicker2').datepicker({ dateFormat: 'mm-dd-yy' }).val();


alert (flight_datepicker1);
alert (flight_datepicker2);*/
