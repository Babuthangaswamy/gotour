...
class Provab_private extends Common_Api_Grind {
    ...
    public function __construct() {
        ...
        $bus_engine_system = $this->CI->config->item('bus_engine_system');

        $this->system = $bus_engine_system;
        $this->UserName = $this->CI->config->item($bus_engine_system . '_username');
        $this->Password = $this->CI->config->item($bus_engine_system . '_password');
        $this->Url = $this->CI->config->item('bus_url');
    ...
    function save_booking($app_booking_id, $params, $module) {
        ...
        $total_fare = array();
        $currency = $params['temp_booking_cache']['book_attributes']['token']['seat_attr']['default_currency'];
       ...
       // debug($SeatFare);exit;
        ...
        if (valid_array($passengers) == true) {
            foreach ($passengers as $key => $passenger) {
                ...
                if ($module == 'b2c') {
                    ...
                    // debug($current_seat_fare);exit;
                    //B2B Calculation
                    $this->commission = $currency_obj->get_commission();
                    //Commission
                    $tieup_agent_commission_percentage = $params['temp_booking_cache']['book_attributes']['token']['CommAmount'];
                    $AgentCommAmount = $this->calculate_commission($tieup_agent_commission_percentage);
                    $admin_commission = $tieup_agent_commission_percentage - $AgentCommAmount;
                    $agent_commission = $AgentCommAmount;
                    //Markup
                    //Admin
                    $admin_markup = $currency_obj->get_currency($current_seat_fare['Fare'], true, true, false);
                    $price_with_admin_markup = $admin_markup['default_value'];
                    $admin_markup = floatval($price_with_admin_markup - $current_seat_fare['Fare']);
                    //Agent
                ...
                $CI->bus_model->save_booking_customer_details($app_reference, $name, $age, $gender, $seat_no, $fare, $status, $seat_type, $is_ac_seat, $admin_commission, $admin_markup, $agent_commission, $agent_markup, $currency, $attr, $admin_tds, $agent_tds);
            }
        }
        ...
        $bus_booking_origin = $CI->bus_model->save_booking_details($domain_origin, $status, $app_reference, $booking_source, $pnr, $ticket, $transaction, $phone_number, $alternate_number, $payment_mode, $created_by_id, $email, $transaction_currency, $currency_conversion_rate, $canacel_policy);
        /**
        ...
        */
        ...
        $GLOBALS ['CI']->transaction->update_convinence_discount_details ( 'bus_booking_details', $app_reference, $discount, $promo_code, $convinence, $convinence_value, $convinence_type, $convinence_per_pax );
        /**
     ...
     */
    function update_markup_currency(& $price_summary, & $currency_obj, $no_of_seats = 1, $level_one_markup = false, $current_domain_markup = true) {
