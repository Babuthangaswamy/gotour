	...
	function format_bus_booking_data($complete_booking_details, $module)
	{
		...
		foreach($master_booking_details as $book_k => $book_v) {
			...
			if($module == 'admin' ){
				...
				$core_booking_details['fare'] = roundoff_number($fare+$admin_commission-$admin_tds);
			}
	...
	function total_fare($fare_details, $module, $currecny='')
	{
		...
		if($module == 'b2c') {//B2C Total Fare
			foreach($fare_details as $k => $v) {
				...
				// echo "fare".$fare.'<br/>';
				// echo "markup".$v['admin_markup'].'<br/>';
				// echo "convinence_amount".$convinence_amount.'<br/>';
				// echo "discount".$discount.'<br/>';
				// echo "admin_commission".$admin_commission.'<br/>';
				// echo "admin_tds".$admin_tds.'<br/>';

				$total_fare[$k] = roundoff_number($fare+$v['admin_markup']+$convinence_amount-$discount-$admin_commission+$admin_tds);
