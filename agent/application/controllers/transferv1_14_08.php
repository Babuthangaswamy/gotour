			...
			$params['op'] == 'get_details' and $safe_search_data['status'] == true) {
				...
				if ($raw_product_deails['status']) {
					...
					if($raw_product_deails['data']['ProductDetails']['TransferInfoResult']['Price']){
						...
						$admin_price_details  = $this->transferv1_lib->update_booking_markup_currency($Price,$currency_obj,$search_id,true,false);
						//echo "amidn";
						//debug($admin_price_details);
						
						$agent_price_details = 	$this->transferv1_lib->update_booking_markup_currency($Price,$currency_obj,$search_id,true,true);
						//echo "agent";
