		  ...
		  <div class="col-xs-12 nopad full_room_buk">
			<div class="bookcol">
			  <div class="hotelistrowhtl">
				<div class="col-md-4 col-xs-12 nopad xcel">
...
if ($total_infant_count > 0) {
	for ($i=$i; $i<=($total_child_count+$total_adult_count+$total_infant_count); $i++) {
		$datepicker_list[] = array('child-date-picker-'.$i, INFANT_DATE_PICKER);
	}
}

if ($total_youth_count > 0) {
	for ($i=$i; $i<=($total_child_count+$total_adult_count+$total_youth_count+$total_infant_count); $i++) {
		$datepicker_list[] = array('child-date-picker-'.$i, YOUTH_DATE_PICKER);
	}
}

if ($total_senior_count > 0) {
	for ($i=$i; $i<=($total_child_count+$total_adult_count+$total_infant_count+$total_youth_count+$total_senior_count); $i++) {
		$datepicker_list[] = array('child-date-picker-'.$i, SENIOR_DATE_PICKER);
	}
}
...
$senior_title_options = $child_title_options;
 ...
 * check if current print index is of adult or child by taking adult and total pax count
 * @param number $total_pax		total pax count
 * @param number $total_adult	total youth count
 */
function is_youth($total_pax, $total_youth)
{
	return ($total_pax>$total_youth ?	false : true);
}
/**
 * check if current print index is of adult or child by taking adult and total pax count
 * @param number $total_pax		total pax count
 * @param number $total_adult	total senior count
 */
function is_senior($total_pax, $total_senior)
{
	return ($total_pax>$total_senior ?	false : true);
}
/**
 * check if current print index is of adult or child by taking adult and total pax count
 * @param number $total_pax		total pax count
 * @param number $total_adult	total infant count
 */
function is_infant($total_pax, $total_infant)
{
	return ($total_pax>$total_infant ?	false : true);
}
/**
 * check if current print index is of adult or child by taking adult and total pax count
 * @param number $total_pax		total pax count
					 		...
					 		<?php foreach($pre_booking_params['BookingQuestions'] as $bq_key=>$bq_value):  ?><!-- start-->
                            	<?php if($bq_value['stringQuestionId']=='weights_passengerWeights'):?>
                            		<?php if($bq_value['required']==1){?>
                            				<div class="col-md-5 nopad">
		                                         ...
		                                         <input class="form-control numeric clainput" type="text" id="weight" placeholder="<?=$bq_value['title']?>" maxlength="3" name="pax_question[<?=$bq_value["questionId"]?>][]">
		                                        </div>		                          
		                                    <div class="col-md-6 spllty">
                            	...
                            	<?php elseif($bq_value['stringQuestionId']=='heights_passengerHeights'):?>
                            		<?php if($bq_value['required']==1):?>
                            			<div class="col-md-5 nopad">
	                                      	<div class="col-md-6 spllty">
	                                         	<input class="form-control clainput numeric" type="text" id="weight" placeholder="Heights" maxlength="3" name="pax_question[<?=$bq_value["questionId"]?>][]">
	                                        </div>
			                                    <div class="col-md-6 spllty">
			                                          <div class="mr-slt-div">
			                                            <?php 
                            	...
                            	<?php elseif($bq_value['stringQuestionId']=='dateOfBirth_dob'):?>
                            		<?php if($bq_value['required']==1):?>
                            			<div class="col-md-2 nopad">
                            			 ...
                            			 <input type="text" class="form-control clainput dobdatepickerbook" id=""  name="pax_question[<?=$bq_value["questionId"]?>][]" placeholder="<?=$bq_value['title']?>" readonly >
                                        </div>
                                        </div>
                            	    <?php endif;?>
                            	<?php elseif($bq_value['stringQuestionId']=='passport_expiry'):?>
                            		<?php if($bq_value['required']==1):?>
                            			 ...
                            			 <input type="text" class="form-control clainput expiraydatepickerbook" id=""  name="pax_question[<?=$bq_value["questionId"]?>][]" placeholder="<?=$bq_value['title']?>" readonly >
                            	    <?php endif;?>
                            	<?php endif;?>
...
<!-- Booking Question  Start-->
		 <?php if(valid_array($pre_booking_params['BookingQuestions'])):?>
                    <?php foreach($pre_booking_params['BookingQuestions'] as $q_key=>$q_value): $requierd='';?>
                      <?php //debug($q_value); ?>
                        <?php if(trim($q_value['stringQuestionId']) !='dateOfBirth_dob' && trim($q_value['stringQuestionId']) !='passport_expiry' && $q_value['stringQuestionId'] !='weights_passengerWeights' && $q_value['stringQuestionId'] !='heights_passengerHeights'):?>
