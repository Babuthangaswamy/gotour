<?php
	...
	$total_room_price  = $pre_booking_params['markup_price_summary']['RoomPrice'];
			        ...
			        <td>
			        	<?php if($Boardingdetails):?>				         		
		         		<?php  
							...
							$b_result = preg_grep('~' . $b_input . '~', $Boardingdetails);
							$board_found =0;
							if($b_result){
								$board_found =1;
								echo '<i class="fa fa-coffee" aria-hidden="true"></i>&nbsp;';
							} 
							$in_search_params = "".strtolower('wireless')."";
							$in_input = preg_quote(@$in_search_params, '~'); // don't forget to quote input string!
							$internet_result = preg_grep('~' . $in_input . '~', $Boardingdetails);
							if($internet_result){
								$board_found =1;
								echo '<i class="fa fa-wifi" aria-hidden="true"></i>&nbsp';
							}else{
								$inn_search_params = "".strtolower('internet')."";
								$inn_input = preg_quote(@$inn_search_params, '~'); // don't forget to quote input string!
								$innternet_result = preg_grep('~' . $inn_input . '~', $Boardingdetails);
								if($innternet_result){
									$board_found =1;
									echo '<i class="fa fa-wifi" aria-hidden="true"></i>&nbsp';
								}else{
									$inn_search_params = "".strtolower('wi-fi')."";
									$inn_input = preg_quote(@$inn_search_params, '~'); // don't forget to quote input string!
									$innternet_result = preg_grep('~' . $inn_input . '~', $Boardingdetails);
									if($innternet_result){
										$board_found =1;
