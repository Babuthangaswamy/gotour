...
<div class="extra_content">
    ...
    <div id="advance_search_form_container" class="serch_area_fltr">
        <form action="<?=base_url().'index.php/report/'.$default_view?>" method="get" autocomplete="off" class="form-horizontal" role="form">
    ...
    <div class="list_of_sections">
		<a class="<?=(($today_search == @$_GET['today_booking_data']) ? 'active' : '')?>" href="<?=base_url().'index.php/report/'.$default_view.'?today_booking_data='.$today_search?>">Today Search</a>
		
		<a class="<?=(($last_today_search == @$_GET['last_day_booking_data']) ? 'active' : '')?>" href="<?=base_url().'index.php/report/'.$default_view.'?last_day_booking_data='.$last_today_search?>">Last Day Search</a>
		<?php 
			$filter_duration_values = array('Last 3 Day Search' => 3,'Last 7 Day Search' => 7,'Last 15 Day Search' => 15,'Last 1 Month Search' => 30,'Last 3 Month Search' => 90);//In days
			foreach($filter_duration_values as $k => $v) { 
			$prev_filter_date = date('Y-m-d', strtotime('-'.intval($v).' day'));
		?>
			<a class="<?=(($prev_filter_date == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url().'index.php/report/'.$default_view.'?prev_booking_data='.$prev_filter_date?>"><?=$k;?></a>
		<?php }
		?>
    </div>
    </div>
